...
"""
def smallest_distance(city_soup, border_cities):
    coordinates_from_link = (city_soup.find('span', class_ = 'geo').text).split(';')
...
def CSVwriter(file):
    ...
    CSV_indx = 0 
    while (CSV_indx < len(cities)):
        CSV_file.writerow([cities[CSV_indx], crimeRate[CSV_indx], distanceToBorder[CSV_indx]])
        CSV_indx += 1
...
while (distance_indx < len(cities)):
    ...
    with open(localURL, encoding = 'utf8') as url:
        ...
        distanceToBorder.append(smallest_distance(city_soup, border_cities))
        distance_indx += 1

#correlation
crimeRateFloat = [float(i) for i in crimeRate]
distanceToBorderFloat = [float(i) for i in distanceToBorder]
correlation = coefficient_of_correlation(crimeRateFloat, distanceToBorderFloat)
